...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: RGB_HEX(0x0081c1);
        
        self.dateLabel.textColor = (blnSelected)?RGB_HEX(0x0081c1):[UIColor whiteColor];
    }else{
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?RGB_HEX(0x7BD1FF):[UIColor whiteColor];
}
