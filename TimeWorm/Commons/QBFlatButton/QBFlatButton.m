...
- (void)drawRect:(CGRect)rect
{
    ...
    [self drawRoundedRect:sideRect radius:self.radius context:UIGraphicsGetCurrentContext() halfSide:YES];
...
- (void)drawRoundedRect:(CGRect)rect radius:(CGFloat)radius context:(CGContextRef)context halfSide:(BOOL)isHalf
{
    rect.origin.x += 0.5;
    rect.origin.y += 0.5;
    rect.size.width -= 1.0;
    ...
    CGContextAddArcToPoint(context, minX, minY, midX, minY, isHalf?rect.size.height/2:radius);
    CGContextAddArcToPoint(context, maxX, minY, maxX, midY, isHalf?rect.size.height/2:radius);
...
- (void)drawRoundedRect:(CGRect)rect radius:(CGFloat)radius context:(CGContextRef)context {
    [self drawRoundedRect:rect radius:radius context:context halfSide:NO];
}
